...
namespace osu.Game.Rulesets.Osu.Difficulty
{
    public class OsuPerformanceCalculator : PerformanceCalculator
    {
        ...
        private double computeReadingHighARValue(ScoreInfo score, OsuDifficultyAttributes attributes)
        {
            ...
            double aimPartValue = highARValue * aimRatio;
            {
                ...
                if (attributes.SliderCount > 0)
                {
                    ...
                    double sliderNerfFactor = (1 - attributes.SliderFactor) * (1 - estimateImproperlyFollowedDifficultSliders / estimateDifficultSliders) + attributes.SliderFactor;
                    aimPartValue *= Math.Pow(sliderNerfFactor, 3);
                }
                ...
                // Scale the aim value with adjusted deviation
                double adjustedDeviation = deviation * calculateDeviationArAdjust(attributes.ApproachRate);
                aimPartValue *= SpecialFunctions.Erf(33 / (Math.Sqrt(2) * adjustedDeviation));
                aimPartValue *= 0.98 + Math.Pow(100.0 / 9, 2) / 2500; // OD 11 SS stays the same.
            }
            ...
            double speedPartValue = highARValue * (1 - aimRatio);
            {
                ...
                // Apply antirake nerf
                double speedAntiRakeMultiplier = calculateSpeedRakeNerf(attributes);
                speedPartValue *= speedAntiRakeMultiplier;
                ...
                speedPartValue *= SpecialFunctions.Erf(20.5 / (Math.Sqrt(2) * adjustedSpeedDeviation));
                speedPartValue *= 0.95 + Math.Pow(100.0 / 9, 2) / 750; // OD 11 SS stays the same.
            }
        ...
        private double calculateBalancerAdjustingMultiplier(ScoreInfo score, OsuDifficultyAttributes osuAttributes)
        {
            ...
            double rescaledValue = (totalValue - 800) / 1000;
            double result = Math.Min(0.08 * rescaledValue, 0.088 * Math.Pow(rescaledValue, 0.4));
            return 1 + result;
        }
