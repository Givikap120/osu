...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators
{
    public static class SpeedEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current)
        {
            ...
            double sliderStreamBonus = GetSliderStreamBonus(current);

            // Base difficulty with all bonuses
            double difficulty = (1 + speedBonus) * (1 + sliderStreamBonus) * 1000 / strainTime;
        ...
        public static double GetSliderStreamBonus(DifficultyHitObject current)
        {
            const double sliderstream_multiplier = 0.25;

            var osuCurrObj = (OsuDifficultyHitObject)current;
            var osuLastObj = (OsuDifficultyHitObject)current.Previous(0);
            var osuLastLastObj = (OsuDifficultyHitObject)current.Previous(1);

            if (osuCurrObj.BaseObject is not Slider slider || osuLastObj?.BaseObject is not Slider || osuLastLastObj == null)
                return 0;
            ...
            return sliderStreamBonus * sliderstream_multiplier;
        }
